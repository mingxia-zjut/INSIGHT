...
from lib.benchmark_utils import get_p_r, get_p_r_all, ransac_pose_estimation, get_inlier_ratio, get_scene_split, write_est_trajectory
...
def get_recognition_p_r(src_loc, tgt_loc, rot, trans):
    src_loc = (torch.matmul(rot, src_loc.transpose(0,1)) + trans).transpose(0,1)
...
def benchmark_predator(feats_scores,n_points,exp_dir,whichbenchmark,sample_method,ransac_with_mutual=False, inlier_ratio_threshold = 0.05):
    ...
    whichbenchmark = 'nodes'
    gt_folder = f'configs/benchmarks_nodes/{whichbenchmark}'
    ...
    results['multi_class_precision_all'] = []
    results['multi_class_recall_all'] = []
    results['accuracy_all'] = []
    results['multi_class_precision'] = []
    results['multi_class_recall'] = []
    results['accuracy'] = []
    tsfm_est = []
    for eachfile in tqdm(feats_scores):
        ...
        tgt_loc = data['tgt_loc']
        src_all_loc = data['src_all_loc']
        tgt_all_loc = data['tgt_all_loc']
        ########################################
        ...
        src_idx = sample_interest_points(sample_method, src_scores,src_pcd.size()[0])
        tgt_idx = sample_interest_points(sample_method, tgt_scores,src_pcd.size()[0])
        ...
        ########################################
        # 3. run ransac
        est = ransac_pose_estimation(src_pcd, tgt_pcd, src_feats, tgt_feats, mutual=ransac_with_mutual)
        tsfm_est.append(est)
        ...
        accuracy, precision, recall = get_p_r(src_loc, tgt_loc, rot, trans, est)
        results['multi_class_precision'].append(float(precision))
        results['multi_class_recall'].append(float(recall))
        results['accuracy'].append(float(accuracy))

        accuracy, precision, recall = get_p_r_all(src_all_loc, tgt_all_loc, rot, trans, est)
        results['multi_class_precision_all'].append(float(precision))
        results['multi_class_recall_all'].append(float(recall))
        results['accuracy_all'].append(float(accuracy))
    ...
    benchmark(exp_dir, gt_folder, results)
...
if __name__=='__main__':  
    ...
    parser.add_argument(
        '--source_path', default="snapshot/indoor/Insight", type=str, help='path to precomputed features and scores')
    # parser.add_argument(
    ...
    parser.add_argument(
        '--n_points', default=30, type=int, help='number of points used by RANSAC')
    # parser.add_argument(
    #     '--exp_dir', default='est_traj', type=str, help='export final results')
    parser.add_argument(
        '--exp_dir', default='snapshot/indoor/est_traj', type=str, help='export final results')
    parser.add_argument(
