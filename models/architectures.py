...
from models.gcn import MultiSpectralAttentionLayer, sel_freq
from models.geo.modules import GeometricTransformerModule
import pickle
...
class KPFCNN(nn.Module):
    ...
    def __init__(self, config):
        ...
        #self.mlp = SharedMLP(2048, 1024, activation_fn=nn.LeakyReLU(0.2))
        #self.leaky_relu = nn.LeakyReLU(0.2)
        #self.FeatureAggregation = LocalFeatureAggregation(1024, 1024, 10, torch.device('cuda'))
        ...
        self.gnn = GCN(num_head,gnn_feats_dim, k, config.nets, self.fre, config)
        """ self.transformer = GeometricTransformerModule(
            256,
            256,
            256,
            4,
            ['self', 'cp', 'cross', 'self', 'cp', 'cross', 'self', 'cp', 'cross'],
            0.2,
            15,
            3
        ) """
        ...
        self.channelattention = MultiSpectralAttentionLayer(1024)
    ...
    def forward(self, batch):
        ...
        if(self.mode == 'test'):
            G0 = self.getGraph(src_pcd_c.unsqueeze(0).transpose(1,2).permute(0,2,1))
            L0 = self.getLaplacian(G0)
            _, u0_0 = torch.symeig(L0.squeeze(0), eigenvectors=True)

            G1 = self.getGraph(tgt_pcd_c.unsqueeze(0).transpose(1,2).permute(0,2,1))
            L1 = self.getLaplacian(G1)
            _, u0_1 = torch.symeig(L1.squeeze(0), eigenvectors=True)
            
            len0 = int((feats_c[:,:,:len_src_c].size()[2]-1)/7)
            len1 = int((feats_c[:,:,len_src_c:].size()[2]-1)/7)
            l1 = []
            l2 = []
            for i in self.fre1:
                l1.append(len0*i)
            for i in self.fre2:
                l2.append(len1*i)
            res0 = sel_freq(feats_c[:,:,:len_src_c], u0_0, l1)
            res1 = sel_freq(feats_c[:,:,len_src_c:], u0_1, l2)

            src = self.channelattention(res0, feats_c[:,:,:len_src_c]).detach().clone()
            feats_c[:,:,:len_src_c] = src
            tgt = self.channelattention(res1, feats_c[:,:,len_src_c:]).detach().clone()
            feats_c[:,:,len_src_c:] = tgt
        ...
        # TODO: 把元数据传送进去
        src_feats_c, tgt_feats_c = self.gnn(src_pcd_c.unsqueeze(0).transpose(1,2), tgt_pcd_c.unsqueeze(0).transpose(1,2),src_feats_c, tgt_feats_c, base, src_name, tgt_name, None, None, None, None, u0_0, u0_1)
        ...
        return feats_f, scores_overlap, scores_saliency, None
