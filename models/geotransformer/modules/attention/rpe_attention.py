...
from models.gcn import SelfAttention, get_vector_and_coord
    ...
    def __init__(self, blocks, d_model, num_head, dropout=0.1, activation_fn='gelu'):
        ...
        for block in self.blocks:
            if block == 'self':
                ...
                layers.append(SelfAttention(256, 10))
            else:
                raise ValueError('Unsupported block type "{}" in `RPEConditionalTransformer`.'.format(block))
        self.layers = nn.ModuleList(layers)

    def forward(self, coords0, coords1, feats0, feats1, embeddings0, embeddings1, masks0=None, masks1=None):
        src_vectors = get_vector_and_coord(coords1.transpose(1,2))
        tgt_vectors = get_vector_and_coord(coords0.transpose(1,2))
        for i, block in enumerate(self.blocks):
            if block == 'self':
                ...
                feats0 = self.layers[i](coords0.transpose(1,2), feats0.transpose(1,2),None, None, None).transpose(1,2)
                #desc0 = layer(coords0, desc0)
                """ coords: torch.Tensor, shape (B, N, 3)
                    coordinates of the point cloud
                    features: torch.Tensor, shape (B, d_in, N, 1)
                    features of the point cloud """
                #max_pool_x = self.mlp(self.FeatureAggregation(coords0.permute(0,2,1), desc0.unsqueeze(-1))).squeeze(0).squeeze(-1).permute(1,0).max(dim=0, keepdim=True)[0]
                feats1 = self.layers[i](coords1.transpose(1,2), feats1.transpose(1,2), None, None, None, None, src_vectors, tgt_vectors, 0).transpose(1,2)
