...
from models.edgescore import EdgeScore
...
class PAConv(nn.Module):
    def __init__(self):
        ...
        i1 = 256  # channel dim of input_1st
        o1 = i2 = 256  # channel dim of output_1st and input_2nd
        o2 = i3 = 512  # channel dim of output_2st and input_3rd
        o3 = i4 = 256  # channel dim of output_3rd and input_4th
        #o4 = 1024  # channel dim of output_4th
        ...
        self.conv5 = nn.Sequential(nn.Conv1d(1024, 256, kernel_size=1, bias=False),
                                   self.bn5)
    ...
    #原本is_src=1
    def forward(self, x, features, label=None, criterion=None, src_vectors=None, tgt_vectors=None, is_src=1):
        B, C, N = x.size()
        k = self.k
        if is_src == 0:
            k = N-1
            sc = self.edgescore(tgt_vectors, src_vectors).repeat(1,self.m1,1,1).permute(0,2,3,1)
        idx, _ = knn(x, k=k)  # different with DGCNN, the knn search is only in 3D space
        xyz = get_scorenet_input(x, idx=idx, k=k)  # ScoreNet input: 3D coord difference concat with coord: b,6,n,k
        ...
        point = torch.cat((point1, point2, point3), dim=1)
