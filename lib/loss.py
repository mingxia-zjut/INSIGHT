...
class MetricLoss(nn.Module):
    ...
    def forward(self, src_pcd, tgt_pcd, src_feats, tgt_feats, correspondence, rot, trans,scores_overlap,scores_saliency, loss):
        """
        ...
        #######################
        # get BCE loss for edge score
        """ gt_labels = torch.zeros(sc.size())
        for i in range(0, sc.size()[2]):
            temp = src_vector[0,:,i].unsqueeze(0).repeat(tgt_vector.size()[2]) """
