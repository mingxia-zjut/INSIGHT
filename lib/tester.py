    ...
    def test(self):
        ...
        with torch.no_grad():
            for idx in tqdm(range(num_iter)): # loop through this epoch
                ...
                data['src_loc'] = inputs['src_loc'].squeeze(0).cpu()
                data['tgt_loc'] = inputs['tgt_loc'].squeeze(0).cpu()
                data['src_all_loc'] = inputs['src_all_loc'].squeeze(0).cpu()
                data['tgt_all_loc'] = inputs['tgt_all_loc'].squeeze(0).cpu()
                ...
                src_idx = sample_interest_points('prob', src_scores,src_pcd.size()[0])
                tgt_idx = sample_interest_points('prob', tgt_scores,src_pcd.size()[0])
                ...
                ########################################
                # 3. run ransac
                ransac_pose_estimation(src_pcd, tgt_pcd, src_feats, tgt_feats, mutual=False) """
