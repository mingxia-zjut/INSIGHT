...
def write_est_trajectory(gt_folder, exp_dir, tsfm_est):
    ...
    for scene_name in scene_names:
        ...
        for i in range(len(gt_pairs)):
            # if int(gt_pairs[i][0]) < len(tsfm_est):
            est_traj.append(tsfm_est[int(gt_pairs[i][0])])
...
def ransac_pose_estimation(src_pcd, tgt_pcd, src_feat, tgt_feat, mutual = False, distance_threshold = 0.05, ransac_n = 3):
    ...
    if(mutual):
        ...
        """ result = o3d.pipelines.registration.registration_icp(
            src_pcd, tgt_pcd, distance_threshold, result_ransac.transformation,
...
def get_p_r_all(src_loc, tgt_loc, rot, trans, est):
    #get label
    src_loc_gt = (torch.matmul(rot, src_loc.transpose(0,1)) + trans).transpose(0,1)
    dist = square_distance(src_loc_gt.unsqueeze(0), tgt_loc.unsqueeze(0))
    gt_idx = dist.topk(k=1, dim=-1, largest=False, sorted=True)[1]
    #print(gt_idx)
    #get est
    crot = copy.deepcopy(est[0:3,0:3])
    ctrans = copy.deepcopy(est[0:3,3])
    src_loc_est = (torch.matmul(torch.from_numpy(crot.reshape(3,3)).float(), src_loc.transpose(0,1)) + torch.from_numpy(ctrans.reshape(3,1)).float()).transpose(0,1)
    dist = square_distance(src_loc_est.unsqueeze(0), tgt_loc.unsqueeze(0))
    est_idx = dist.topk(k=1, dim=-1, largest=False, sorted=True)[1]
    mx = torch.zeros([tgt_loc.size()[0],tgt_loc.size()[0]])
    for i in range(0, src_loc.size(0)):
        #mx[est_idx[0,i,0], gt_idx[0,i,0]] = 1
        mx[est_idx[0,i,0], i] = 1
    precision = 0
    recall = 0
    for i in range(src_loc.size(0)):
        #l = gt_idx[0,i,0]
        TP = mx[i, i]
        if TP == 0:
            continue
        FN = torch.sum(mx[:,i])-mx[i,i]
        FP = torch.sum(mx[i,:])-mx[i,i]
        precision += TP/(TP+FP)
        recall += TP/(TP+FN)
    
    accuracy = 1
    for i in range(src_loc.size(0)):
        #if est_idx[0,i,0] != gt_idx[0,i,0]:
        if est_idx[0,i,0] != i:
            accuracy = 0

    return accuracy, precision / src_loc.size(0), recall / src_loc.size(0)

def get_p_r(src_loc, tgt_loc, rot, trans, est):
    #get label
    src_loc_gt = (torch.matmul(rot, src_loc.transpose(0,1)) + trans).transpose(0,1)
    dist = square_distance(src_loc_gt.unsqueeze(0), tgt_loc.unsqueeze(0))
    gt_idx = dist.topk(k=1, dim=-1, largest=False, sorted=True)[1]
    ...
    for i in range(src_loc.size(0)):
        ...
        if TP == 0:
            continue
        FN = torch.sum(mx[:,l])-mx[l,l]
        FP = torch.sum(mx[l,:])-mx[l,l]
        precision += TP/(TP+FP)
        recall += TP/(TP+FN)
...
def get_scene_split(whichbenchmark):
    ...
    folder = f'configs/benchmarks_nodes/{whichbenchmark}/*/gt.log'
