...
def benchmark(est_folder,gt_folder,results):
    ...
    with open(f'{est_folder}/result','w') as f:
        ...
        f.write("multi_class_precision: {:.3f}\n".format(np.sum(np.asarray(results['multi_class_precision'])) / len(results['multi_class_precision'])))
        f.write("multi_class_recall: {:.3f}\n".format(np.sum(np.asarray(results['multi_class_recall'])) / len(results['multi_class_recall'])))
        f.write("accuracy: {:.3f}\n".format(np.sum(np.asarray(results['accuracy'])) / len(results['accuracy'])))
        f.write("multi_class_precision_all: {:.3f}\n".format(np.sum(np.asarray(results['multi_class_precision_all'])) / len(results['multi_class_precision_all'])))
        f.write("multi_class_recall_all: {:.3f}\n".format(np.sum(np.asarray(results['multi_class_recall_all'])) / len(results['multi_class_recall_all'])))
        f.write("accuracy_all: {:.3f}\n".format(np.sum(np.asarray(results['accuracy_all'])) / len(results['accuracy_all'])))
